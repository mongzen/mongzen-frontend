...
import { useServicePage } from '@/hooks/useApi';
    ...
    <div className="min-h-screen bg-neutral-60">
      ...
      <section className="border-x-0 sm:border-x border-b border-dark-15">
        <div className="mx-auto">
          <PageHeader
            title={
              serviceData?.ServiceProjectCTA.title ||
              'Ready to Start Your Project?'
            }
            subtitle={
              serviceData?.ServiceProjectCTA.subtitle ||
              'Let us help you bring your vision to life'
            }
            backgroundImage={formatImageUrl(
              serviceData?.ServiceProjectCTA.background?.url
            )}
            icon={formatImageUrl(serviceData?.ServiceProjectCTA.icon?.url)}
            className="lg:py-[120px] lg:space-y-10 space-y-4 py-10 !border-b-0 md:px-16 xl:px-[350px] px-10"
          >
            <Link href={serviceData?.ServiceProjectCTA.button_link || '#'}>
              <WipeButton
                variant="filled"
                color="primary"
                size="lg"
              ...
              >
                {serviceData?.ServiceProjectCTA.button_text || 'Get Started'}
              </WipeButton>
            </Link>
          </PageHeader>
        </div>
